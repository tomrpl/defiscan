...
Aerodrome provides multiple access points for users, including both centralized interfaces ([aerodrome.finance](aerodrome.finance), [alt.aerodrome.finance](alt.aerodrome.finance)) and decentralized interfaces on IPFS ([aero.drome.eth](aero.drome.eth), [aero.drome.eth.limo](aero.drome.eth.limo), [aero.drome.eth.link](aero.drome.eth.link)). This diversity in user interfaces ensures redundancy, allowing users to access the protocol even if one interface becomes unavailable. 
