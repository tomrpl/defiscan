      ...
      <table className="table-auto w-full border-collapse">
        ...
        <tbody>
          {protocols.map((protocol) => {
            const protocolData = data !== undefined ? data.filter(e => e.slug == protocol.defillama_slug)[0] : 'Loading...';
            return (
              <tr
                key={protocol.slug}
              ...
              >
                <td className="border-b px-4 py-2">
                  <img src={protocolData != undefined ? protocolData.logo : "/images/placeholder.png"} alt={protocol.protocol} className="w-16 h-16 object-cover" />
                </td>
                <td className="border-b px-4 py-2">{protocol.protocol}</td>
                <td className="border-b px-4 py-2">{protocol.type}</td>
                <td className="border-b px-4 py-2">{"Stage " + protocol.stage}</td>
                <td className="border-b px-4 py-2">{protocol.risks}</td>
                <td className="border-b px-4 py-2">
                  {protocolData !== undefined ? formatUsd(protocolData.tvl) : 'N/A'}
                </td>
              </tr>
          )})}
        </tbody>
